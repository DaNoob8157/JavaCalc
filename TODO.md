
# JavaCalc-Class Project TODO

## Core Features
| Done | Task | Assigned/Dibs |
|------|------|------------|
| ☐ | Implement basic arithmetic operations (add, subtract, multiply, divide) |  |
| ☐ | Add support for decimal numbers and floating-point calculations |  |
| ☐ | Implement order of operations (PEMDAS/BODMAS) |  |
| ☐ | Add parentheses support for complex expressions |  |
| ☐ | Implement clear/reset functionality |  |

## Advanced Operations
| Done | Task | Assigned/Dibs |
|------|------|-------------|
| ☐ | Add trigonometric functions (sin, cos, tan) | |
| ☐ | Implement power and square root operations | |
| ☐ | Add percentage calculations | |
| ☐ | Implement memory functions (M+, M-, MR, MC) | |
| ☐ | Support for scientific notation | |

## UI/UX Improvements
| Done | Task | Assigned/Dibs |
|------|------|---------------|
| ☐ | Design professional GUI layout |               |
| ☐ | Add keyboard input support |               |
| ☐ | Implement button feedback (visual/audio) | `@danoob8157` |
| ☐ | Display calculation history |               |
| ☐ | Add keyboard shortcuts for operations |               |
| ☐ | Implement dark/light theme toggle |               |

## Error Handling
| Done | Task | Assigned/Dibs |
|------|------|-------------|
| ☐ | Handle division by zero gracefully | |
| ☐ | Validate user input | |
| ☐ | Display error messages clearly | |
| ☐ | Implement input sanitization | |
| ☐ | Handle overflow/underflow cases | |

## Code Quality
| Done | Task | Assigned/Dibs |
|------|------|-------------|
| ☐ | Add unit tests for CalculatorEngine | |
| ☐ | Create integration tests for Controller | |
| ☐ | Add JavaDoc comments to all classes | |
| ☐ | Refactor large methods into smaller functions | |
| ☐ | Implement proper exception handling | |

## Documentation
| Done | Task | Assigned/Dibs |
|------|------|-------------|
| ☐ | Complete README.md with usage instructions | |
| ☐ | Add architecture diagram | |
| ☐ | Create user guide | |
| ☐ | Document all public methods | |
| ☐ | Add code comments for complex logic | |

## Optional Enhancements
| Done | Task | Assigned/Dibs |
|------|------|-------------|
| ☐ | Add currency conversion feature | |
| ☐ | Implement expression history saving to file | |
| ☐ | Create undo/redo functionality | |
| ☐ | Add unit conversion support | |
| ☐ | Implement RPN (Reverse Polish Notation) mode | |